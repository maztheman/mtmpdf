...
bool CPdfToExcel::ProcessRows(const TextAtLocation& text_data, int& iLastCol)
{
...
static const TextAtLocation* FindPageText(const vector<TextAtLocation>& texts, const TextAtLocation* pCurrentLocation)
{
    for (auto& text : texts) {
...
static const TextAtLocation* FindLineItemTotals(const vector<TextAtLocation>& texts)
{
    for (auto& text : texts) {
...
void CPdfToExcel::SetupColumns(std::vector<const TextAtLocation*>& header_items)
{
...
void CPdfToExcel::TraceBackForTrade(const vector<TextAtLocation>& texts, size_t header_index)
{
...
const TextAtLocation* FindNextTradeItem(const vector<TextAtLocation>& all_texts, size_t first, size_t last)
{
...
vector<HorizontalLine*> CPdfToExcel::CollectHorizontalLines(size_t nPage)
{
...
const HorizontalLine* CPdfToExcel::DoesTextHaveAUnderline(const TextAtLocation* text, const vector<HorizontalLine*>& lines, const struct Rectangle* mb)
{
...
vector<const TextAtLocation*> GetThisPagesTextNodes(const vector<TextAtLocation>& all_texts, size_t nPage)
{
    vector<const TextAtLocation*> retval;
...
void CPdfToExcel::ProcessAllTextNodes(const vector<TextAtLocation>& all_texts)
{
    ...
    if (TryFindAllDescription(all_texts, arDescrIndexs)) {
        ...
        for (size_t n = 0; n < arDescrIndexs.size(); n++) {
            ...
            for (size_t row = first; row != last; ++row) {
                ...
                bool rc = ProcessRows(all_texts[row], iLastCol);
            }
        }
    }
}
